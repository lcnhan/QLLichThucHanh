...
<body>
                    ...
                    <div class="page-inner py-5">
                        <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
                            <div>
                                ...
                                <select name="slc_hk" id="slc_hk"> 
                                    @foreach($allHK as $all_hk) 
                                        <option value="{{$all_hk->id_hk}}" @if($hk==$all_hk->id_hk) selected @endif>Học kỳ {{$all_hk->ky_hieu}} - {{$all_hk->nien_khoa}}
                                        </option> 
                                    @endforeach 
                                </select>
                            </div>
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        <button type="button" class="btn_control text-primary" data-toggle="modal" data-target="#edit" onclick="editData('{{$lhp->ma_lophp}}'); changePName('{{$lhp->ma_lophp}}', '{{$lhp->ten_lophp}}', '{{$lhp->ma_mon}}', '{{$lhp->so_buoi_th}}', '{{$lhp->ma_canbo}}', '{{$lhp->id_hk}}', '{{$lhp->thu}}', '{{$lhp->tietbd}}' );">
                    ...
                    <div class="modal-content ">
                        ...
                        <div class="modal-body">
                            <form action="{{ route('addLHP') }}" method="POST">
                                ...
                                <div class="form-group form-group-default col-12 col-sm-12 col-md-12 col-lg-12" id="updateHK">
    ...
    $("#slc_hk").change(function() {
        var hkid = $('#slc_hk').val();
        console.log('loading: ' + hkid);
        $.ajax({
            url: "{{ asset('/loadDSLHP_HK') }}",
            method: "GET",
            data: { 'hk_id': hkid },
            success: function(data) {
                ...
                $('#basic-datatables').DataTable({});
                window.history.pushState('page2', 'Title', '/dslhp/' + hkid);
                $("div#updateHK select").val(""+hkid);
